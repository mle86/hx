...
use vars qw(
	...
	$re_time $re_ddd $re_ms $re_dmy $re_ymd $re_ddmyt $re_dmdty $re_dmdyt $re_ts8601 $re_tsw $re_tsv $re_sects $re_dmyts $re_ymdts $re_unixts
...
my  $re_relpath = qr/(?:(?!(?:[A-Za-z]+.?|\d+)(?:$|\s))(?:${re_pathchr}+:?\/+)*[A-Za-z0-9\-_\.\+\$]+)/;
...
our $re_ddmyt  = qr/(?:[A-Z][A-Za-z]{1,2},?  ?\d\d?(?:st|nd|rd|th)?,? [A-Z][A-Za-z]{2} \d\d\d\d,? ${re_time}${re_ms}?(?: $re_tz)?+)/;  # Wed 08 Sep 2021 22:00:00 CEST
our $re_dmdyt  = qr/(?:[A-Z][A-Za-z]{1,2},? [A-Z][A-Za-z]{2}  ?\d\d?(?:st|nd|rd|th)?,? \d\d\d\d,? ${re_time}${re_ms}?(?: $re_tz)?+)/;  # Sun Apr 26 2020 13:49:59 GMT
our $re_dmdty  = qr/(?:[A-Z][A-Za-z]{1,2},? [A-Z][A-Za-z]{2}  ?\d\d?(?:st|nd|rd|th)?,? ${re_time}${re_ms}?,? \d\d\d\d)/;  # Sun Apr 26 13:49:59 2020
...
our $re_unixts = qw/(?:\d{9,}(?:\.\d*)?)/;  # 1633877183  or  1633877183.37411
